...
class Dice(Metrics):
    ...
    def update(self, ground, predict):
        ...
        pred = torch.argmax(pred, dim=1).long()#(torch.argmax(pred, dim=1)+1) * (torch.max(pred,dim=1)[0] > 0.5).long()
        gr = torch.argmax(gr, dim=1).long()#(torch.argmax(gr, dim=1)+1) * (torch.max(gr,dim=1)[0] > 0.5).long()
...
class Hausdorff_ITK(Metrics):
    ...
    def update(self, ground, predict):
        ...
        pred = torch.argmax(pred, dim=1).long().cpu().numpy()#pred = torch.argmax(pred, dim=1).cpu().numpy()#((torch.argmax(pred, dim=1)+1) * (torch.max(pred,dim=1)[0] > 0.5).long()).cpu().numpy()
        gr = torch.argmax(gr, dim=1).long().cpu().numpy()#gr = torch.argmax(gr, dim=1).cpu().numpy()#((torch.argmax(gr, dim=1)+1) * (torch.max(gr,dim=1)[0] > 0.5).long()).cpu().numpy()
